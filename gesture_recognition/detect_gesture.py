...
def gesture_setup(member, model_name, scaler_name):
  model = joblib.load('models/' + member + '/' + model_name + ".joblib")
  scaler = joblib.load('models/' + member + '/' + scaler_name + ".joblib")

  IMU.detectIMU()
  IMU.initIMU()
  return model, scaler

def get_gesture(max_time, model, scaler):
